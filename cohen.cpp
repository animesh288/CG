...
int main()
{
    ...
    else
    {
        ...
        {
            y = y1 + (float)(W_xmin - x1) * slope;
            x = W_xmin;
        }
        if (rcode_begin[2] == 1) // right
        {
            y = y1 + (float)(W_xmax - x1) * slope;
            x = W_xmax;
        }
        if (rcode_begin[0] == 1) // top
        {
            x = x1 + (float)(W_ymax - y1) / slope;
            y = W_ymax;
        }
        if (rcode_begin[1] == 1) // bottom
        {
            x = x1 + (float)(W_ymin - y1) / slope;
            y = W_ymin;
        }
        // end points
        if (rcode_end[3] == 1) // left
        {
            y1 = y + (float)(W_xmin - x) * slope;
            x1 = W_xmin;
        }
        if (rcode_end[2] == 1) // right
        {
            y1 = y + (float)(W_xmax - x) * slope;
            x1 = W_xmax;
        }
        if (rcode_end[0] == 1) // top
        {
            x1 = x + (float)(W_ymax - y) / slope;
            y1 = W_ymax;
        }
        if (rcode_end[1] == 1) // bottom
        {
            x1 = x + (float)(W_ymin - y) / slope;
            y1 = W_ymin;
        }
    }
    delay(1000);
